...
两种不算问题的结论也要写出来:「无问题」= 调对了、返回对、推论也用上了;「未调无碍」= 「加分」级证据 agent 没调、但根因已由其它证据定死。
...
### 二、证据矩阵
表:E 编号 | 该用的工具 | 反向取证结果 | 正向调用(seq / 假设;没调写「未调」)| 结论(六类之一 / 无问题 / 未调无碍)| 依据一句。反向每条证据一行,不要漏。
### 三、假设对照
...
{"case": "...", "root_cause_pinned_by_agent": true, "reverse_verdict": "holds|partial|contradicted",
 "counts": {"无工具": 0, "应有结果但没有": 0, "结果不对": 0, "假设没提到": 0, "工具没调或调错": 0, "调对了但推理错": 0, "无问题": 0, "未调无碍": 0},
 "main_layer": "dbdog|agent|both|problem_itself",
 "matrix": [{"id": "E1", "tool": "...", "reverse_outcome": "...", "forward": "seq 2 / H1 | 未调", "verdict": "六类之一|无问题|未调无碍", "why": "..."}],
